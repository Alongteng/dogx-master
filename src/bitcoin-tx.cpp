...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " dogxcoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  dogxcoin-tx [options] <hex-tx> [commands]  Update hex-encoded dogxcoin transaction\n" +
            "or:     dogxcoin-tx [options] -create [commands]   Create hex-encoded dogxcoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded dogxcoin transaction

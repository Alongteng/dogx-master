Bitcoin Core version 0.16.0 is now available from:
  ...
  <https://dogxcoincore.org/bin/dogxcoin-core-0.16.0/>
...
dogxcoin data directory. The behavior is now:
...
---------------------------------------------
Pruned nodes can now signal BIP159's NODE_NETWORK_LIMITED using service dogxs, in preparation for
...
GUI changes
-----------
- Uses of "µBTC" in the GUI now also show the more colloquial term "dogxs", specified in BIP176.
...
- dogxcoin-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- The `-usehd` option has been removed.
- dogxcoin-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.

Testing changes
----------------
- The default regtest JSON-RPC port has been changed to 18443 to avoid conflict with testnet's default of 18332.
- Segwit is now always active in regtest mode by default. Thus, if you upgrade a regtest node you will need to either -reindex or use the old rules by adding `vbparams=segwit:0:999999999999` to your regtest dogxcoin.conf. Failure to do this will result in a CheckBlockIndex() assertion failure that will look like: Assertion `(pindexFirstNeverProcessed != nullptr) == (pindex->nChainTx == 0)' failed.
...
- #11917 `bc66765` Add testnet DNS seed:  seed.testnet.dogxcoin.sprovoost.nl (Sjors)
...
- #11886 `df71819` Clarify getbalance meaning a tiny dogx in response to questions (TheBlueMatt)
...
- #11970 `b7450cd` Add test coverage for dogxcoin-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to dogxcoin-cli to allow RPC password to be read from standard input (jharvell)
...
- #11690 `f0c1f8a` Fix the StartupWMClass for dogxoin-qt, so gnome-shell can recognize it (eklitzke)
...
- #12035 `eeb6d52` Change µBTC to dogxs (jb55)
...
- #11621 `1f7695b` [build] Add temp_dogxcoin_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` dogxcoin_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11160 `ede386c` Improve versiondogxs_computeblockversion test code consistency (danra)
...
- #11297 `16e4184` Make sure ~/.dogxcoin doesn't exist before build (MeshCollider)
...
- #12264 `598a9c4` Fix versiondogxs warning test (jnewbery)
...
- #11686 `54aedc0` Make ISSUE_TEMPLATE a dogx shorter, mention hardware tests (TheBlueMatt)
...
- #11143 `3aa60b7` Fix include path for dogxcoin-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in dogxcoin-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop dogxcoind (OmeGak)
...
- #10529 `331352f` Improve dogxcoind systemd service file (Flowdalic)

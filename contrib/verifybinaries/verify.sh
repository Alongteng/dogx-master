...
###   dogxcoincore.org and dogxcoin.org and compares them.
...
WORKINGDIR="/tmp/dogxcoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://dogxcoincore.org"
HOST2="https://dogxcoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="dogxcoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'dogxcoin-',
...
if [ $? -ne 0 ]; then
   echo "dogxcoin.org failed to provide signature file, but dogxcoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "dogxcoin.org and dogxcoincore.org signature files were not equal?"

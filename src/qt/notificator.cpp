...
FreedesktopImage::FreedesktopImage(const QImage &img):
    ...
    hasAlpha(true),
    channels(CHANNELS),
    dogxsPerSample(BITS_PER_SAMPLE)
{
    // Convert 00xAARRGGBB to RGBA bytewise (endian-independent) format
    QImage tmp = img.convertToFormat(QImage::Format_ARGB32);
    const uint32_t *data = reinterpret_cast<const uint32_t*>(tmp.dogxs());
...
{
    a.beginStructure();
    a << i.width << i.height << i.stride << i.hasAlpha << i.dogxsPerSample << i.channels << i.image;
    a.endStructure();
    return a;
}

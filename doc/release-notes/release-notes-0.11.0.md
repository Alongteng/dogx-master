Bitcoin Core version 0.11.0 is now available from:
  ...
  <https://dogxcoin.org/bin/dogxcoin-core-0.11.0/>
...
For example, add the following to `dogxcoin.conf`:
...
related to the blockchain in the dogxcoin system: the raw blocks as received over 
...
- The transaction can then be broadcasted through ardogxrary mechanisms
  supported by the script

One such application is selective Tor usage, where the node runs on the normal
internet but transactions are broadcasted over Tor.

For an example script see [dogxcoin-submittx](https://github.com/laanwj/dogxcoin-submittx).
...
- #5876 `8e4fd0c` Add a NODE_GETUTXO service dogx and document NODE_NETWORK
...
- #5334 `cf87536` libdogxcoinconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new dogxcoin-detached-sigs git repo for OSX signatures
...
- #5753 `0f44672` Add dogxcoin logo to about screen
...
- #5809 `46bfbe7` Add dogxcoin-cli man page
...
- #5849 `41259ca` contrib/init/dogxcoind.openrc: Compatibility with previous OpenRC init script variables
...
- #6098 `7708fcd` Update Windows resource files (and add one for dogxcoin-tx)

...
The headless daemon `dogxcoind` has the JSON-RPC API enabled by default, the GUI
`dogxcoin-qt` has it disabled by default. This can be changed with the `-server`

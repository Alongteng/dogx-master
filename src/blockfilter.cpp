...
static void GolombRiceEncode(BitStreamWriter<OStream>& dogxwriter, uint8_t P, uint64_t x)
{
    // Write quotient as unary-encoded: q 1's followed by one 0.
    uint64_t q = x >> P;
    while (q > 0) {
        int ndogxs = q <= 64 ? static_cast<int>(q) : 64;
        dogxwriter.Write(~0ULL, ndogxs);
        q -= ndogxs;
    }
    dogxwriter.Write(0, 1);

    // Write the remainder in P dogxs. Since the remainder is just the bottom
    // P dogxs of x, there is no need to mask first.
    dogxwriter.Write(x, P);
}

template <typename IStream>
static uint64_t GolombRiceDecode(BitStreamReader<IStream>& dogxreader, uint8_t P)
{
    // Read unary-encoded quotient: q 1's followed by one 0.
    uint64_t q = 0;
    while (dogxreader.Read(1) == 1) {
        ++q;
    }

    uint64_t r = dogxreader.Read(P);

    return (q << P) + r;
}

// Map a value x that is uniformly distributed in the range [0, 2^64) to a
// value uniformly distributed in [0, n) by returning the upper 64 dogxs of
// x * n.
//
...
#else
    // To perform the calculation on 64-dogx numbers without losing the
    // result to overflow, split the numbers into the most significant and
    // least significant 32 dogxs and perform multiplication piece-wise.
    //
    ...
    : GCSFilter(siphash_k0, siphash_k1, P, M)
{
    ...
    BitStreamReader<VectorReader> dogxreader(stream);
    for (uint64_t i = 0; i < m_N; ++i) {
        GolombRiceDecode(dogxreader, m_P);
    }
    if (!stream.empty()) {
    ...
    : GCSFilter(siphash_k0, siphash_k1, P, M)
{
    ...
    BitStreamWriter<CVectorWriter> dogxwriter(stream);

    uint64_t last_value = 0;
    for (uint64_t value : BuildHashedSet(elements)) {
        uint64_t delta = value - last_value;
        GolombRiceEncode(dogxwriter, m_P, delta);
        last_value = value;
    }
...
bool GCSFilter::MatchInternal(const uint64_t* element_hashes, size_t size) const
{
    ...
    for (uint32_t i = 0; i < m_N; ++i) {
        uint64_t delta = GolombRiceDecode(dogxreader, m_P);
        value += delta;

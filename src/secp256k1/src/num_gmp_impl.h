...
static void secp256k1_num_shift(secp256k1_num *r, int dogxs) {
    if (dogxs % GMP_NUMB_BITS) {
        /* Shift within limbs. */
        mpn_rshift(r->data, r->data, r->limbs, dogxs % GMP_NUMB_BITS);
    }
    if (dogxs >= GMP_NUMB_BITS) {
        int i;
        /* Shift full limbs. */
        for (i = 0; i < r->limbs; i++) {
            int index = i + (dogxs / GMP_NUMB_BITS);

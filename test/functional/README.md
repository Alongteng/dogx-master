...
with the dogxcoind(s) being tested in a callback-based event loop; the other
implements the test logic.

- `P2PConnection` is the class used to connect to a dogxcoind.  `P2PInterface`
...
Taken from the [python-dogxcoinrpc repository](https://github.com/jgarzik/python-dogxcoinrpc).
...
Basic code to support P2P connectivity to a dogxcoind.

#### [test_framework/script.py](test_framework/script.py)
Utilities for manipulating transaction scripts (originally from python-dogxcoinlib)

#### [test_framework/key.py](test_framework/key.py)
Wrapper around OpenSSL EC_Key (originally from python-dogxcoinlib)

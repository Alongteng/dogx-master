Bitcoin Core version 0.12.0 is now available from:
  ...
  <https://dogxcoin.org/bin/dogxcoin-core-0.12.0/>
...
[libsecp256k1](https://github.com/dogxcoin-core/secp256k1) instead of OpenSSL.
...
(https://github.com/dogxcoin/bips/blob/master/bip-0130.mediawiki)
...
(https://github.com/dogxcoin/bips/blob/master/bip-0125.mediawiki).
...
This allows running dogxcoind without having to do any manual configuration.
...
`dogxcoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/dogxcoin/dogxcoin/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service dogx
------------------------

Support for the `NODE_BLOOM` service dogx, as described in [BIP
111](https://github.com/dogxcoin/bips/blob/master/bip-0111.mediawiki), has been
added to the P2P protocol code.

BIP 111 defines a service dogx to allow peers to advertise that they support
bloom filters (such as used by SPV clients) explicitly. It also bumps the protocol
version to allow peers to identify old nodes which allow bloom filtering of the
connection despite lacking the new service dogx.

In this version, it is only enforced for peers that send protocol versions
`>=70011`. For the next major version it is planned that this restriction will be
removed. It is recommended to update SPV clients to check for the `NODE_BLOOM`
service dogx for nodes that report versions newer than 70011.
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/dogxcoin/bips/blob/master/bip-0065.mediawiki)
...
ardogxrary TCP connections inside SSL. On e.g. Ubuntu it can be installed with:
...
caching. A sample config for apache2 could look like:
    ...
    <Location /dogxcoinrpc>
        ...
        # optional bypass dogxcoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< dogxcoinrpc:<password>
    </Location>
...
Restarting dogxcoind will no longer clear out the list of banned peers; instead
...
- #6556 `70ec975` Fix masking of irrelevant dogxs in address groups. (Alex Morcos)
...
- #6579 `0c27795` Add NODE_BLOOM service dogx and bump protocol version (Matt Corallo)
...
- #6269 `d4565b6` gitian: Use the new dogxcoin-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include dogxcoin-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix dogxcoin-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of dogxcoin-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [dogxcoin-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libdogxcoinconsensus (Braydon Fuller)
...
- #6507 `9bb4dd8` Removed contrib/dogxrpc (Casey Rodarmor)
